...
// 注意：PreparedStatement不直接支持注册用户回调值，需要使用回调方式处理或在之后使用批处理来获取返回值

// 执行存储过程
stmt->execute();
// 获取存储过程返回结果，使用查询方式获取存储过程的返回值，这里需要通过查询获取
// 例如，在存储过程的结果集中通过@result获取返回值，可以在之后查询获取。
...
pstmt->setString(1, name); // 将username替换为需要查询的用户名
// 执行查询
std::unique_ptr<sql::ResultSet> res(pstmt->executeQuery());
