...
set(CHATSERVER_ALLOW_DEBUG_ONLY_DEPS ON CACHE BOOL "Allow Debug libraries to satisfy non-Debug configurations when Release libs are unavailable")

function(chatserver_import_library target_name debug_path)
  set(options)
  ...
  elseif(CHATSERVER_ALLOW_DEBUG_ONLY_DEPS)
    set_target_properties(${target_name} PROPERTIES
    ...
    message(FATAL_ERROR
      ...
      "Either provide the Release library or enable CHATSERVER_ALLOW_DEBUG_ONLY_DEPS."
    )
  endif()
endfunction()

add_library(ChatServer::third_party INTERFACE IMPORTED GLOBAL)

set(_chatserver_include_dirs
...
set_target_properties(ChatServer::third_party PROPERTIES
  INTERFACE_INCLUDE_DIRECTORIES "${_chatserver_include_dirs}"
)

chatserver_import_library(ChatServer::jsoncpp
  "${JSON_ROOT}/lib/json_vc71_libmtd.lib"
  RELEASE_PATH "${JSON_ROOT}/lib/json_vc71_libmt.lib"
)

chatserver_import_library(ChatServer::protobuf
...
foreach(lib IN ITEMS gpr grpc grpc++ grpc++_reflection address_sorting upb)
  ...
  chatserver_import_library("ChatServer::${_sanitized}"
    "${GRPC_VISUALPRO_ROOT}/Debug/${lib}.lib"
    RELEASE_PATH "${GRPC_VISUALPRO_ROOT}/Release/${lib}.lib"
  )
  list(APPEND _chatserver_link_targets "ChatServer::${_sanitized}")
endforeach()

chatserver_import_library(ChatServer::cares
...
chatserver_import_library(ChatServer::zlib
...
chatserver_import_library(ChatServer::ssl
...
chatserver_import_library(ChatServer::crypto
...
foreach(lib IN LISTS _absl_components)
  ...
  chatserver_import_library("ChatServer::${lib}" "${_absl_dir}/Debug/${lib}.lib" RELEASE_PATH "${_absl_dir}/Release/${lib}.lib")
  list(APPEND _chatserver_link_targets "ChatServer::${lib}")
endforeach()

list(APPEND _chatserver_link_targets
  ChatServer::jsoncpp
  ChatServer::protobuf
  ChatServer::cares
  ChatServer::zlib
  ...
  ChatServer::re2
  ChatServer::hiredis
  ChatServer::win32_interop
  ChatServer::mysqlcppconn
  ChatServer::mysqlcppconn8
  ChatServer::boost_filesystem
  ws2_32
)

set_target_properties(ChatServer::third_party PROPERTIES
  INTERFACE_LINK_LIBRARIES "${_chatserver_link_targets}"
)
